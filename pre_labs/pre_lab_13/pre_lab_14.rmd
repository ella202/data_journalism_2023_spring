...
**Answer** It describes who each state voted for and their percentage in favor of Trump in the presidential election and the amount of loan paid out to employees. Also how many businesses in each state received the PPP loans. 
```{r}
...
**Answer** There's a diagonal line going through a chart in order to help translate positive or negative correlation according to the placements of the dots.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** With a flatter correlation line, there seems to be a more neutral relationship between Trump voting states and loans distributed to employees.
```{r}
...
**Answer** Correlation points appear between different comparisons. 
...
**Answer** The accuracy of the correlation.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** The confidence level of the correlation.
```{r}
...
**Answer** There are differences in numbers but we don't know if they're meaningful
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** Our first  t-test to test accuracy of the means.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** There is a significannce between PPP loans distributed to states who voted vor Trump and Clinton in the 2026 president election.
```{r}

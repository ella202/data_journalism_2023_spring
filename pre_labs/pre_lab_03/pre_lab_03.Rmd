...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** No.
...
**Answer** There are 547 rows with fewer than five character. The file was opened in excel and the zeros disappeared because that's what happens when data is produced there.
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer** There are categories that are missing and random letters as categories to name a couple.
...
**Answer** The government lent $60, which is a really small loan.
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer** A new data frame is created, and names of columns are easier to use because we don't have to use quotes when they're used.
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer** 50 duplicates
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer** the dataframe has 17,740 rows.
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** the zipcodes longer than five digits were cut off and place into a news variable.This probably means they wouldn't be considered characters anymore if that was the case before. 
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? 21202 -- Baltimore!

```{r}
cleaned_md_grants_loans %>% 
 filter(source == 'COMMERCE/MARYLAND TOURISM BOARD') %>%
  group_by(zip5) %>% 
  summarise(total_amount = sum(amount),) %>% arrange(desc(total_amount))

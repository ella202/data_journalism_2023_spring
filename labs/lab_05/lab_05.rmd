...
```{r}
 od_log <- read_csv("tabula-Baltimore County; Carey, Samantha log OD.csv", col_names = FALSE)  %>% clean_names() %>%
...
A1. July 14 had the most calls with 46. There was no day without calls in the data but it could have just been left out or unrecorded. 
...
A2. The percentages are pretty close together, not really seeing an outlier. The most overdose calls occur on Saturdays with 1244 of them. Thursdays have the least calls with 1004.
...
od_logs <- od_log %>%
  ...
  mutate(percent_of_calls = num_calls / nrow(od_logs) * 100)
```
...
A3. The most calls take place in what looks to be a single family home. There are 72 calls that have taken place here which is quite high. Looking at the original data, there were calls on the sheet that were duplicates in that they occurred in the same location and at the same time. This might have thrown the data off because it's strange how two calls could have been made at once down to the second. 
...
A4. I would definitely want to know why there are two calls happening occurring at the same time. I would want to maybe write about the response times of overdose 911 calls can shed light on the effectiveness of the county's response to overdoses -- that would also require some more digging.

...
**A1.** chart: [https://datawrapper.dwcdn.net/30Fdn/1/] I made the char because it clearly shows the difference in voter turnout and performance. Clearly there was a difference in votes between hogan and Cox in their elections. 
...
```
```{r}
cox_hogan <- md_gov_county %>% left_join(md_gov_county_18, by=c("County" = "County"), ("GEOID" = "GEOID"))
```

```{r}
cox_hogan2 <- cox_hogan %>%
   mutate(total_votes = Cox + Moore + Lashar + Wallace + Harding + `Write-ins.x`) %>%
  mutate(pct_Cox = Cox/total_votes*100) %>%

  mutate(total_votes2 = Hogan + Jealous + Quinn + Schlakman + `Write-ins.y`) %>%
     mutate(pct_Hogan= Cox/total_votes2*100)
```

```{r}
cox_hogan3 <- cox_hogan2 %>%
  select(County, pct_Hogan, pct_Cox)
```
```{r}
write_csv(cox_hogan3, "data/cox_hogan3.csv")
```
...
**A2.** https://datawrapper.dwcdn.net/pZf4q/1/ I think the story is the fact that in-person election day voting hasn't made a come back since the COVID-19 pandemic began. 
```{r}
md_votes_by_type.csv <- read_csv("data/md_votes_by_type.csv")
...
**A3.** https://datawrapper.dwcdn.net/X8Djy/2/ Zip codes in the central part of the county have higher numbers of od calls. I wander if the population is bigger in these zip codes and that could be the reason. 

```{r}
montgomery_2022_overdoses <- read_csv("data/montgomery_2022_overdoses.csv")
```
```{r}
montgomery_od_2022_totalszips <- montgomery_2022_overdoses %>%
  group_by(zip) %>% 
summarize(zip_calls = n())
```

```{r}
mc_ods_2022 <- montgomery_od_2022_totalszips %>% 
   summarize(zip_calls_sum = sum(zip_calls)) %>% 

 mutate(pct_zip = zip_calls/zip_calls_sum*100)
```


write_csv(montgomery_od_2022_totalszips, "data/montgomery_od_2022_totalszips.csv")
```